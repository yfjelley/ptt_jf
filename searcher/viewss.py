    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if pw is not None and _code == int(smscode):
                ...
                form.valiatetype(2)
                return render_to_response('forgetpwd_m.html',{"form":form},
                                      context_instance=RequestContext(request))

        else:
            return render_to_response('forgetpwd_m.html', {'form': form}, context_instance=RequestContext(request))
    else:
        form = ForgetPWForm()
        return render_to_response('forgetpwd_m.html', {'form': form}, context_instance=RequestContext(request))
...
def safecenter(request):
    ...
    if request.method =="POST":
        ...
        print "safecenter"
        form = ModfiyPWForm()
        t = get_template('safecenter_m.html')
        content_html = t.render(
...
def change_phone_number(request):
    if request.method == "POST":
        ...
        form = ModfiyPForm()
        t = get_template('changephone_m.html')
        content_html = t.render(
                RequestContext(request,{'form':form}))
...
def prodetails(request,objectid):
    ...
    return render_to_response('prodetails_m.html',{"flag":flag,"result":p,"project_forum":forum,'amount':amount,'invest_detail':invest_de}, context_instance=RequestContext(request))
...
def invest_pr(request,objectid):
    ...
    return render_to_response('invest_pr_m.html',{"invest_type":int(objectid[-1]),"project":p}, context_instance=RequestContext(request))
