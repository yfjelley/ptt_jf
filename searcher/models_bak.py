...
class invest_detail(models.Model):
    invest_user = models.ForeignKey("投资人姓名",User,related_name = "invest_user_set", blank=True, null=True)
    invest_project = models.ForeignKey("项目",Project,related_name = "invest_project_set", blank=True, null=True)
    invest_amount = models.CharField("每股金额",max_length=255, blank=True, null=True)#投资金额
    invest_num = models.CharField("股数",max_length=255, blank=True, null=True)#投资份数
...
class project_forum(models.Model):
    forum_user = models.ForeignKey("留言者",User,related_name = "forum_user_set", blank=True, null=True)
    forum_project = models.ForeignKey("项目",Project,related_name = "forum_project_set", blank=True, null=True)
    forum_content = models.CharField("内容",max_length=255, blank=True, null=True)#内容
    time_created = models.DateTimeField(auto_now_add=True)

class Signal(models.Model):
    type = models.IntegerField("类型",default=0)        # 0,系统公告；1:评论;2:私信;3.关注;4.主题关注
    obj = models.IntegerField("对象",default=0)         # 对象id
    user = models.ForeignKey("发布者",User,related_name = "signal_user_set", blank=True, null=True)      # 发布者
    who = models.ForeignKey("接受者",User,related_name = "signal_who_set", blank=True, null=True)                   # 接受者
    title = models.CharField("标题",max_length=200, null=True)        # 标题
    content = models.CharField("内容",max_length=1000, null=True)        # 内容
